...
inline std::shared_ptr<spdlog::logger> spdlog::daily_date_logger_st(const std::string& logger_name, const filename_t& filename, int hour, int minute)
{
	return create<spdlog::sinks::daily_date_file_sink_st>(logger_name, filename, hour, minute);
}
